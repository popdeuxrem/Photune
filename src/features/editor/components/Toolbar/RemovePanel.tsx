...
export function RemovePanel() {
  ...
  const handleMagicErase = async () => {
    ...
    const bg = fabricCanvas.backgroundImage as fabric.Image | undefined;
    if (!bg) return;
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Inpaint failed with status ${res.status}`);
      }
      ...
      fabric.Image.fromURL(
        ...
        (img) => {
          ...
          window.setTimeout(() => URL.revokeObjectURL(cleanedUrl), 30_000);
        },
        { crossOrigin: 'anonymous' }
      );
    } catch {
      updateJob(id, 'failed', 'Magic Erase failed. Original image preserved.');
    } finally {
